<?php
/*
Plugin Name: WordPress OAuth
Plugin URI: http://github.com/xtremefaith/wordpress-oauth
Description: A WordPress plugin that allows users to login or register by authenticating with an existing Google, Facebook, LinkedIn, Github, Reddit, Outlook 365 or Windows Live account via OAuth 2.0. Easily drops into new or existing sites, integrates with existing users.
Version: 0.1.0
Author: Nick Worth
...
/**
 * Start the user session for persisting user/login 
 * state during ajax, header redirect, and cross domain calls
 */
if (!isset($_SESSION)) session_start();

/**
 * WordPress OAuth Object
 */
Class WPOA {

	/**
	 * Plugin version should always be set prior to 
	 * a new release
	 *
	 * @var        string
	 */
	...
	/**
	 * Define the settings used by this plugin; this array 
	 * will be used for registering settings, applying default
	 * values, and deleting them during uninstall
	 * 
	 * TODO: Separate provider settings to be set during provider setup
	 *
	 * @var        array
	 */
	...
	*/
	...
	function wpoa_update_notice() {
		$settings_link = "<a href='options-general.php?page=wordpress-oauth.php'>Settings Page</a>"; // CASE SeNsItIvE filename!
		?>
		<div class="updated">
			<p>WordPress OAuth has been updated! Please review the <?php echo $settings_link ?>.</p>
		</div>
		<?php
	}
	...
	function wpoa_init_frontend_scripts_styles() {
		...
		wp_enqueue_script('wpoa-script', plugin_dir_url( __FILE__ ) . 'wordpress-oauth.js', array());
		wp_enqueue_style('wpoa-style', plugin_dir_url( __FILE__ ) . 'wordpress-oauth.css', array());
	}
	...
	function wpoa_settings_link($links) {
		$settings_link = "<a href='options-general.php?page=wordpress-oauth.php'>Settings</a>"; // CASE SeNsItIvE filename!
	...
	/**
	 * Load the provider script that is being requested by the user
	 ...
	 * TODO: this is usually called from a custom logout button, but 
	 * 		 we could have the button call /wp-logout.php?action=logout for
	 * 		 more consistency...
	 */
	...
	/**
	 * Returns a saved login form design as a shortcode atts string or array 
	 * for direct use via the shortcode
	 *
	...
	/**
	 * Check if a login form design exist
	 *
	 * @param      string  $design_name
	 *
	 * @return     boolean
	...
	function wpoa_settings_page() {
		add_options_page( 'WordPress OAuth Options', 'WordPress OAuth', 'manage_options', 'wordpress-oauth', array($this, 'wpoa_settings_page_content') );
	}
	...
	function wpoa_settings_page_content() {
		...
		include 'wordpress-oauth-settings.php';
	}
} // END OF WPOA CLASS
